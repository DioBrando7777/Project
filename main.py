...
def main():
    ...
    Predictor = dlib.shape_predictor(
        "shape_predictor_68_face_landmarks.dat")  # instantiation of the dlib keypoint detector model
    '''
    ...
    Eye_det = EyeDet(show_processing=True)
    ...
    while True:  # infinite loop for webcam video capture
        ...
        if delta_time >= time_lim:  # if the time passed is bigger or equal than the frame time, process the frame
            ...
            if len(faces) > 0:  # process the frame only if at least a face is found
                ...
                if asleep:
                    cv2.putText(frame, "ASLEEP!", (10, 310),
                                cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1, cv2.LINE_AA)
                if looking_away:
                ...
                if(ear>0.27):
                    ...
                    sleepy=0
                    if(active>10):
                        cv2.putText(frame, "active", (100,340),cv2.FONT_HERSHEY_COMPLEX,2,(255,0,0),2,cv2.LINE_AA)
                        
                elif(ear<0.27 and ear>0.21):
                    drowsy+=1
                    active=0
                    sleepy=0
                    if(drowsy>10):
                        cv2.putText(frame, "Drowsy", (100,340),cv2.FONT_HERSHEY_COMPLEX,2,(255,0,0),2,cv2.LINE_AA)
                        beep_sound=pygame.mixer.Sound("beep-01a.mp3")
                        beep_sound.play()
                else:
                    sleepy+=1
                    drowsy=0
                    active=0
                    if(sleepy>10):
                        cv2.putText(frame, "sleepy", (100,340),cv2.FONT_HERSHEY_COMPLEX,2,(255,0,0),2,cv2.LINE_AA)
                        beep_sound=pygame.mixer.Sound("beep-01a.mp3")
                        beep_sound.play()
